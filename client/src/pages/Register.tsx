...
export default function Register() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err.response?.data?.error || 'Registration failed. Grid blocked.');
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="min-h-screen flex items-center justify-center p-6 bg-background text-foreground relative overflow-hidden">
      ...
      >
        <div className="text-center mb-10">
          ...
          <p className="text-slate-400 text-lg">Initialize your unique neural node.</p>
        </div>
        ...
        <div className="glass-card p-10 rounded-[2.5rem] border border-white/10 shadow-2xl relative group">
          <div className="relative">
            ...
            <form className="space-y-6" onSubmit={handleSubmit}>
              <div>
                <label className="block text-sm font-black text-slate-500 mb-3 uppercase tracking-widest">Formal Alias</label>
                <input
                  type="text"
                  required
              ...
              <div>
                <label className="block text-sm font-black text-slate-500 mb-3 uppercase tracking-widest">Access Logic</label>
                <input
                  type="password"
                  required
                  className="w-full px-6 py-4 bg-white/5 border border-white/10 rounded-2xl text-white placeholder-slate-600 focus:outline-none focus:ring-2 focus:ring-teal-500/50 focus:border-teal-500 transition-all text-lg"
